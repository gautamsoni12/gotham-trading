...
import pandas as pd
import talib
from queries import StrategyUnoQueries
...
def calculate_indicators(stock_data: pd.DataFrame) -> pd.DataFrame:
    ...
    bbands_upper, bbands_middle, bbands_lower = talib.BBANDS(stock_data['close'], timeperiod=20)
    
    stoch, stoch_signal = talib.STOCH(stock_data['high'], stock_data['low'], stock_data['close'],fastk_period=14, slowk_period=3, slowk_matype=0, slowd_period=3, slowd_matype=0)
...
def get_data_to_insert(stock_symbol, indicator_df: pd.DataFrame):
        return [(stock_symbol, row['close'], row['date'], row['RSI'], row['MACD'], row['MACD_Signal'], row['macd_signal_line'], row['ADX'],  
            row['BBands_Upper'], row['BBands_Middle'], row['BBands_Lower'], row['Stoch'], row['Stoch_Signal'], row['RSI_Signal'], row['ADX_Signal'], row['BBands_Signal'] , row['trade_decision'], row['seven_day_price_diff'], row['seven_day_price_diff_percent']) for index, row in indicator_df.iterrows()]
