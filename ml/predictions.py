...
def make_predictions_and_save(stock):
    try:
        ...
        two_days_ago = datetime.today().date() - timedelta(days=8)
        ...
        X_pred = prediction_data.drop(columns=['stock_symbol', 'date' , 'close', 'seven_day_price_diff', 'seven_day_price_diff_percent', 'trade_decision'])
        ...
        # # Get the confidence level for the positive class
        # confidence = proba[:, 1]
